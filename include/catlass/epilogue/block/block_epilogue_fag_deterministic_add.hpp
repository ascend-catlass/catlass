...
#ifndef CATLASS_EPILOGUE_BLOCK_EPILOGUE_FAG_DETERMINISTIC_ADD_HPP
#define CATLASS_EPILOGUE_BLOCK_EPILOGUE_FAG_DETERMINISTIC_ADD_HPP
...
class BlockEpilogue<EpilogueAtlasA2FAGDtmAdd<INPUT_LAYOUT_>, OutputType_, UpdateType_, InputType_> {
public:
    ...
    CATLASS_DEVICE
    void CalcDqReduce(
        DBParams& dbParam, GlobalTensor<float>& srcTensor, GlobalTensor<float>& dstTensor, int64_t d, int64_t dAlign,
        uint32_t vecCalBlockNum)
    {
        pingpongIdx = dbParam.taskId % 2;
        // 整块切分
        uint32_t s1CalcInner = (s1CvInner + vecCalBlockNum - 1) / vecCalBlockNum;
        int64_t singleCoreDataNum = s1CalcInner * dAlign; // dAlign是16对齐

        LocalTensor<float> dqRes = unifiedBuffer.GetWithOffset<float>(
            singleCoreDataNum, 0); // GetWithOffset(长度，偏移：单位字节，需32字节对齐)
        LocalTensor<float> inBuf =
            unifiedBuffer.GetWithOffset<float>(singleCoreDataNum, singleCoreDataNum * sizeof(float));

        for (int8_t groupId = 0; groupId < cubeCoreNum; groupId++) { // cubeCoreNum=coreNum/2
    ...
    CATLASS_DEVICE
    BlockEpilogue(
        Arch::Resource<ArchTag>& resource, __gm__ uint8_t* actual_seq_qlen, __gm__ uint8_t* actual_seq_kvlen,
        __gm__ uint8_t* workspace, const Params& params, TBuf<>& buf)
    {
        unifiedBuffer = buf;
    ...
    CATLASS_DEVICE
    void operator()(
        DBParams& dbParam, GlobalTensor<float>& dqWorkSpaceGm, GlobalTensor<float>& dkWorkSpaceGm,
        ...
        GlobalTensor<float>& dvDtmWsGm)
    {
        ...
        if (unlikely(
                s1CalcInner * dAlign * sizeof(float) * 2 > TOTAL_SIZE ||
