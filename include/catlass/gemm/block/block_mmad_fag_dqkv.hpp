#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FULL_LOADB_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FULL_LOADB_HPP
...
template <
    uint32_t L1A_STAGES_, uint32_t L1B_STAGES_, bool ENABLE_UNIT_FLAG_, class BlockTileShape_, class L1ATileShape_,
    class L1BTileShape_, class L0TileShape_, class AType_, class BType_, class CType_, class BiasType_, class TileCopy_,
    class TileMmad_>
struct BlockMmadFAG<
    MmadAtlasA2FagdQKV<L1A_STAGES_, L1B_STAGES_, ENABLE_UNIT_FLAG_>, BlockTileShape_, L1ATileShape_, L1BTileShape_,
...
public:
    ...
    static_assert(
        (BlockTileShape::N == 64) || (BlockTileShape::N == 128) || (BlockTileShape::N == 192) ||
            (BlockTileShape::N == 256),
        "BlockN must be in (64, 128, 192, 256)!");

    // Check Layout
    static_assert(
        std::is_same_v<LayoutA, layout::RowMajor> || std::is_same_v<LayoutA, layout::ColumnMajor>,
    ...
    static_assert(
        std::is_same_v<LayoutAInL1, layout::zN> || std::is_same_v<LayoutAInL1, layout::nZ>,
    ...
    CATLASS_DEVICE
    BlockMmadFAG(
        Arch::Resource<ArchTag>& resource, uint32_t l1BufAddrStart = 0, uint32_t eventIdStart = 0) // eventIdStart给0或4
    {
