...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_FAG_SDP_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_FAG_SDP_HPP
...
template <
    uint32_t L1A_STAGES_, uint32_t L1B_STAGES_, bool ENABLE_UNIT_FLAG_, class BlockTileShape_, class L1TileShape_,
...
struct BlockMmadFagSdp<
    MmadAtlasA2FagSdp<L1A_STAGES_, L1B_STAGES_, ENABLE_UNIT_FLAG_>, BlockTileShape_, L1TileShape_, L0TileShape_, AType_,
...
public:
    ...
    static_assert(
        (BlockTileShape::K == 64) || (BlockTileShape::K == 128) || (BlockTileShape::K == 192) ||
    ...
    // Check Layout
    static_assert(
        std::is_same_v<LayoutA, layout::RowMajor>, "LayoutA only support RowMajor yet!"); // A矩阵仅支持[M, K]格式
    static_assert(
    ...
    /// Construct
    CATLASS_DEVICE
    BlockMmadFagSdp(Arch::Resource<ArchTag>& resource, uint32_t l1BufAddrStart = 0)
    {
...
#endif // CATLASS_GEMM_BLOCK_BLOCK_MMAD_FAG_SDP_HPP
